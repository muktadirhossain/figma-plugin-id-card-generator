        ...
        if (msg.images && Array.isArray(msg.images)) {
          for (const imageData of msg.images) {
            if (imageData.name && imageData.data) {
              ...
              console.log(
                `Stored image: ${imageData.name} (${uint8Array.length} bytes)`
              );
            }
          }
        }
...
async function generateIDCards(options: {
  ...
  try {
    ...
    for (let i = 0; i < csvData.length; i++) {
      ...
      if (frontTemplate) {
        const frontCard = await generateSingleCard(
          frontTemplate.node,
          rowData,
          "front",
          i + 1,
      ...
      if (backTemplate) {
        const backCard = await generateSingleCard(
          backTemplate.node,
          rowData,
          "back",
          i + 1,
        ...
        const totalFrontWidth =
          options.cardsPerRow * (frontWidth + options.spacing);
      ...
      const currentCardName = `${rowData["id"] || rowData["ID"] || i + 1}-${
        rowData["username"] ||
        rowData["name"] ||
        rowData["Name"] ||
        `user${i + 1}`
...
async function generateSingleCard(
  ...
  const username =
    data["username"] || data["name"] || data["Name"] || `user${cardNumber}`;
...
async function populateCardData(
  ...
  if (node.type === "TEXT") {
    ...
    if (data.hasOwnProperty(layerName)) {
      ...
      console.log(
        `Found image reference for "${layerName}": ${imageReference}`
      );
      ...
      if (imageReference && imageReference.trim() !== "") {
        ...
        if (imageData) {
          try {
            ...
            console.warn(
              `Failed to load uploaded image ${imageReference}:`,
              error
            );
          }
        }
        ...
        if (
          ...
          try {
            ...
            console.warn(
              `Failed to load image from URL ${imageReference}:`,
              error
            );
          }
        }
        ...
        console.warn(
          `Could not load image "${imageReference}" - not found in uploads and not a valid URL`
        );
...
async function handleNameField(textNode: TextNode, options: TextResizeOptions) {
  try {
    ...
    console.log(
      `Name field: "${originalText}" - Container: ${containerWidth}x${containerHeight}, Actual: ${actualTextWidth}x${actualTextHeight}`
    );
    ...
    if (overflowsWidth || overflowsHeight) {
      console.log(
        `Name field overflow detected, converting to two lines: "${originalText}"`
      );
      ...
      if (
        typeof originalLineHeight === "object" &&
        ...
        if (originalLineHeight.unit === "PERCENT") {
          ...
          const lineHeight = originalLineHeight as {
            value: number;
            unit: "PIXELS";
          };
      ...
      if (
        tempTextNode.width <= containerWidth &&
        ...
        console.log(
          `Successfully applied two-line format for name: "${multiLineText}"`
        );
      } else {
        console.log(
          `Two-line name still doesn't fit, keeping original format: "${originalText}"`
        );
    ...
    console.warn(
      `Name field handling failed for "${textNode.characters}":`,
      error
    );
...
async function autoResizeText(textNode: TextNode, options: TextResizeOptions) {
  try {
    ...
    const isNameField =
      layerName.includes("name") || layerName.includes("username");
    ...
    console.log(
      `Text: "${textNode.characters}" - Container: ${containerWidth}x${containerHeight}, Actual: ${actualTextWidth}x${actualTextHeight}`
    );
    ...
    if (overflowsWidth || overflowsHeight) {
      ...
      if (isNameField && overflowsWidth && textLength > options.nameThreshold) {
        console.log(
          `Attempting to convert long name to multi-line: "${textNode.characters}"`
        );
        ...
        if (
          typeof currentLineHeight === "object" &&
          ...
          if (currentLineHeight.unit === "PERCENT") {
            ...
            const lineHeight = currentLineHeight as {
              value: number;
              unit: "PIXELS";
            };
            tempTextNode.lineHeight = {
        ...
        if (
          ...
          console.log(
            `Multi-line name still doesn't fit, reverting to original text`
          );
          tempTextNode.characters = textNode.characters;
      ...
      let newFontSize = Math.floor(
        originalFontSize * scaleFactor * safetyMargin
      );
      ...
      console.log(
        `Resizing font from ${originalFontSize}px to ${newFontSize}px (min: ${minFontSize}px) for "${layerName}"`
      );
      ...
      if (
        ...
        if (
          ...
          const lineHeight = tempTextNode.lineHeight as {
            value: number;
            unit: "PIXELS";
          };
          const newLineHeight = Math.max(
            newFontSize * 1.1,
            lineHeight.value * 0.9
          );
        ...
        if (
          ...
          tempTextNode.fontSize = newFontSize;
          console.log(
            `Final font size adjustment to ${newFontSize}px for "${layerName}"`
          );
        }
      }
...
async function loadImageFromData(
  ...
  try {
    console.log(
      `Loading image from uploaded data, size: ${imageData.length} bytes`
    );
    ...
    console.log(
      `Successfully applied uploaded image to rectangle "${rectNode.name}" with scale mode: ${imageScaling}`
    );
  } catch (error) {
...
async function loadImageFromUrl(
  ...
  try {
    ...
    rectNode.fills = newFills;
    console.log(
      `Successfully applied image to rectangle "${rectNode.name}" with scale mode: ${imageScaling}`
    );
  } catch (error) {
    console.error(`Failed to load image from ${imageUrl}:`, error);
    createImagePlaceholder(
      rectNode,
      `Failed to load: ${imageUrl.substring(0, 30)}...`
    );
    throw error;
  }
}
